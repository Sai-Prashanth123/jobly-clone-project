...
export default function ClientSLA() {
  ...
  const { data: clients = [], isLoading } = useClientSLA(month);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-5">
      ...
      {isLoading ? <div className="flex justify-center py-16"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div> : clients.length === 0 ? (
          ...
          {clients.map((c: any) => (
            <div key={c.clientId} className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${badge(c.fulfillmentPct)}`}>{c.fulfillmentPct}% fulfilled</span>
              </div>
